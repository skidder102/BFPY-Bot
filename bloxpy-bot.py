...
async def predict(ctx):
    ...
    if ctx.author.id != bot.user.id:
        await ctx.reply(embed=discord.Embed(description="Check DMs",color=0x5ca3ff))
        ok = await ctx.author.send(embed=discord.Embed(title="Checking BFAPI",description="Checking API",color=0x5ca3ff))
        def lol():
...
bot.run("MTAwNTYyNjQzNDY1NzcyMjM4OA.GuGTdg.kg6bm4o_jf2i1Qqn30bOeWwD8byU8R3lcBcxak")
